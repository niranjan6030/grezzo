...
export async function POST(req: Request) {
  ...
  const events: BrowseEvent[] = Array.isArray(body.events) ? body.events.slice(-60) : [];
  const favourites: string[] = Array.isArray(body.favourites) ? body.favourites : [];
  const cart: string[] = Array.isArray(body.cart) ? body.cart : [];
