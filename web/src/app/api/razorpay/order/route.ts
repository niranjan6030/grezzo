...
function summariseLines(lines: { sku: string; qty: number }[]): string {
...
export async function POST(req: Request) {
  ...
  const bankOfferId: string | undefined = typeof body.bankOfferId === "string" ? body.bankOfferId : undefined;
  const method: PaymentMethod = body.method ?? "upi";

  if (method === "cod") {
    return NextResponse.json(
      { error: "Cash on delivery is placed through /api/orders/cod." }, { status: 400 });
  }
  ...
  const recordLocally = async (razorpayOrderId?: string) => {
