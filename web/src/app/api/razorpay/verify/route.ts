...
export async function POST(req: Request) {
  ...
  const markLocally = async (status: "paid" | "signature_failed") => {
    if (db) return null;
    const { result } = await updateAdminData((draft) => {
      ...
      if (status === "paid") {
        ...
        (o.timeline ??= []).push({
          status: "paid", at: o.paidAt, note: "Payment received",
        });
      }
      return o.id;
    });
