...
export async function GET() {
  ...
  const revenue = (list) => list.reduce((n, o) => n + o.totalPaise, 0);
  const units = (list) => list.reduce((n, o) => n + o.lines.reduce((m, l) => m + l.qty, 0), 0);
    ...
    stats: {
      ...
      bestSellers: [...sold.entries()]
        ...
        .map(([productId, qty]) => ({
          productId,
          qty,
          name: catalogue.find((p) => p.id === productId)?.name ?? productId,
        })),
