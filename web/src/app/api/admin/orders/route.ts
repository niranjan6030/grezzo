...
export async function PATCH(req: Request) {
  ...
  if (db) {
    ...
    if (!(FLOW[row.status] ?? []).includes(status)) {
      return NextResponse.json(
        { error: `Cannot move an order from ${row.status} to ${status}.` }, { status: 409 });
    }
    const entry = { status, at: new Date().toISOString(), note: NOTE[status] };
    const { error } = await db.rpc("append_order_status", {
      p_order_id: id, p_status: status, p_entry: entry,
    });
  ...
  const { result } = await updateAdminData((draft) => {
    ...
    o.status = status;
    (o.timeline ??= []).push({
      status, at: new Date().toISOString(), note: NOTE[status],
    });
    return "ok";
  });

  if (result === "missing") return NextResponse.json({ error: "Order not found." }, { status: 404 });
  if (result === "illegal") return NextResponse.json({ error: "That status change is not allowed." }, { status: 409 });
  return NextResponse.json({ ok: true });
}
