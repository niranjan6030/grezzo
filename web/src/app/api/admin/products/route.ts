...
export async function PATCH(req: Request) {
  ...
  if (Array.isArray(body.tags)) {
    patch.tags = body.tags.map((t: unknown) => String(t).trim().toLowerCase())
  ...
  if (Array.isArray(body.colours)) {
    ...
    for (const c of body.colours) {
      ...
      colours.push({
        code: String(c.code).toLowerCase().replace(/[^a-z0-9-]/g, "-").slice(0, 32),
        wash: c.wash,
        ramp: [String(c.ramp[0]), String(c.ramp[1]), String(c.ramp[2])],
      });
    }
    if (colours.length === 0) {
      return NextResponse.json({ error: "A product needs at least one colourway." }, { status: 400 });
    }
  ...
  if (body.photos && typeof body.photos === "object") {
    ...
    for (const [code, url] of Object.entries(body.photos as Record<string, unknown>)) {
