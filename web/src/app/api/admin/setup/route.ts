...
export async function GET() {
  ...
  if (!db) {
    out.push({
      id: "supabase", name: "Database (Supabase)", state: "off",
      detail: "Using the local file store. Fine for development; edits vanish on a serverless deploy.",
    });
  } else {
    const { error } = await db.from("site_config").select("key").limit(1);
    out.push(error
      ? { id: "supabase", name: "Database (Supabase)", state: "broken",
          detail: `Connected but the query failed: ${error.message}. Has schema.sql been run?` }
      : { id: "supabase", name: "Database (Supabase)", state: "live",
  ...
  if (!keyId || !keySecret) {
    out.push({
      id: "razorpay", name: "Payments (Razorpay)", state: "off",
      detail: keySecret
    ...
    try {
      ...
      out.push({
        id: "razorpay", name: "Payments (Razorpay)", state: "live",
      ...
      out.push({
        id: "razorpay", name: "Payments (Razorpay)", state: "broken",
  ...
  /* ---- webhook ---- */
  out.push(process.env.RAZORPAY_WEBHOOK_SECRET
    ? { id: "webhook", name: "Payment webhook", state: "live",
        detail: "Signed webhooks accepted — this is what confirms payment authoritatively." }
    : { id: "webhook", name: "Payment webhook", state: "off",
        detail: "No signing secret. Orders confirm on the browser callback alone, which is weaker." });

  /* ---- AI service ---- */
  const ai = process.env.AI_SERVICE_URL;
  if (!ai) {
    out.push({
      id: "ai", name: "AI service", state: "off",
    ...
    try {
      ...
      out.push({
        id: "ai", name: "AI service", state: "live",
        detail: `Recommender: ${json.recommender}. Vision: ${json.vision}.`,
      });
    } catch {
      out.push({
        id: "ai", name: "AI service", state: "broken",
