      ...
      >
        <div className="flex items-center justify-between px-5 py-4 md:px-10">
          ...
          <div className="flex items-center gap-4 md:gap-6">
            <button onClick={() => setSearchOpen((v) => !v)} aria-label="Search"
              ...
              {searchOpen ? <X size={20} strokeWidth={1.25} /> : <Search size={20} strokeWidth={1.25} />}
            </button>

            {/* The lens — image search sits next to the search field, as asked. */}
            <button onClick={() => openLens()} aria-label="Search by photo"
                    className="group relative transition-opacity hover:opacity-55">
              <Camera size={20} strokeWidth={1.25} />
              <span className="absolute -right-1 -top-1 h-1.5 w-1.5 rounded-full bg-thread
                               animate-pulse" />
            </button>

            <Link href="/favourites" aria-label="Favourites" className="relative transition-opacity hover:opacity-55">
              <Heart size={20} strokeWidth={1.25} />
              {favCount > 0 && <Badge n={favCount} light={overHero} />}
            </Link>

            <Link href="/account" aria-label="Account" className="hidden transition-opacity hover:opacity-55 sm:block">
              <User size={20} strokeWidth={1.25} />
            </Link>

            <Link href="/cart" aria-label="Cart" className="relative transition-opacity hover:opacity-55">
        ...
        <AnimatePresence>
          {searchOpen && (
            ...
            >
              <form onSubmit={submit} className="mx-auto flex max-w-3xl items-center gap-3 px-5 py-6">
                ...
                />
                <button type="button" onClick={() => { setSearchOpen(false); openLens(); }}
                ...
                <div className="mx-auto max-w-3xl px-5 pb-6">
                  {suggestions.map((p) => (
                    <Link key={p.id} href={`/product/${p.slug}`}
                      ...
                      <span>{p.name}</span>
                      <span className="text-ink-soft">{p.fit} · {p.wash}</span>
                    </Link>
                  ))}
                </div>
              )}
          ...
          >
            ...
            <div className="flex flex-wrap gap-x-8 gap-y-2 px-6 py-8 md:px-16">
              {[...ALL_FITS.slice(0, 5), ...ALL_WASHES.slice(0, 4)].map((t) => (
                <Link key={t} href={`/jeans?q=${encodeURIComponent(t)}`}
                      className="tracked text-ink-soft seam-link">{t}</Link>
              ))}
            </div>
          </motion.div>
        )}
...
function Badge({ n, light }: { n: number; light: boolean }) {
  return (
    <span className={`absolute -right-2 -top-1.5 flex h-4 min-w-4 items-center justify-center
