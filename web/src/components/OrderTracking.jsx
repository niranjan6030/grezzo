...
const STAGES = [
  { key: "placed", label: "Order placed", icon: Check, hint: "We have your order" },
  {
    ...
    label: "Confirmed",
    icon: Package,
    hint: "Payment settled, picking has started",
  },
...
export default function OrderTracking({ receipt }) {
  ...
  if (!user) {
    return (
      <Centered>
        ...
        <Link
          href={`/account?next=/account/orders/${receipt}`}
  ...
  return (
    <section className="mx-auto max-w-3xl px-5 py-16 md:px-10">
      <Link href="/account/orders" className="tracked text-ink-soft seam-link">
        ← All orders
      </Link>
            ...
            return (
              <li key={s.key} className="relative flex gap-5 pb-10 last:pb-0">
                ...
                <div className="pt-1">
                  ...
                  <p className="mt-1.5 text-sm text-ink-soft">{entry?.note ?? s.hint}</p>
                  {entry && (
      ...
      <div className="mt-12 flex flex-col gap-3 sm:flex-row">
        ...
        <Link href="/contact" className="tracked px-6 py-3.5 text-center text-ink-soft seam-link">
