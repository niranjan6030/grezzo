...
export default function Dashboard() {
  ...
  const series = useMemo(() => {
    ...
    return buckets.map((paise, i) => ({
      paise,
      label: new Date(startMs + i * 86_400_000)
  ...
  return (
    <>
      ...
      {backend === "file" && (
        <Card className="mb-8 border-thread/40 bg-thread/8 p-5">
          ...
          <p className="mt-2.5 text-sm leading-relaxed text-ink-soft">
            ...
            That file is fine locally but serverless hosts have a read-only, temporary
            filesystem — connect Supabase before going live or your changes will vanish
            on the next deploy.
          </p>
        </Card>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Stat label="Revenue — 24h" value={rupees(stats.revenueDayPaise)}
              hint={`${stats.ordersDay} order${stats.ordersDay === 1 ? "" : "s"}`} />
        <Stat label="Revenue — 7d" value={rupees(stats.revenueWeekPaise)} />
        <Stat label="Average order" value={rupees(stats.averageOrderPaise)}
              hint={`${stats.unitsAll} units all time`} />
        <Stat label="Abandoned checkouts" value={String(stats.abandoned)}
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            ...
            <Link href="/admin/orders" className="tracked seam-link text-ink-soft">All</Link>
          </div>
          {recent.length === 0 ? (
        ...
        <div className="space-y-6">
          <Card className="p-6">
            <div className="flex items-center justify-between">
              ...
              <Link href="/admin/inventory" className="tracked seam-link text-ink-soft">Manage</Link>
            </div>
                ...
                <div className="mt-5 divide-y divide-line">
                  {stats.lowStock.slice(0, 6).map((s) => (
                    <div key={s.sku} className="flex items-center justify-between py-2.5">
                      <p className="text-sm">
                        {s.name}
                        <span className="ml-2 text-xs text-ink-soft">{s.colour} · {s.size}</span>
                      </p>
                  ...
                  return (
                    <div key={b.productId}>
                      ...
                      <div className="mt-1.5 h-[3px] bg-denim-wash">
                        <div className="h-full bg-denim-deep"
...
export function StatusPill({ status }: { status: string }) {
  const tone =
    status === "paid" || status === "delivered" ? "good"
    : status === "shipped" || status === "cod_pending" ? "neutral"
    : status === "created" ? "warn"
    : "bad";
  return <Pill tone={tone as "good" | "neutral" | "warn" | "bad"}>{status.replace("_", " ")}</Pill>;
}
