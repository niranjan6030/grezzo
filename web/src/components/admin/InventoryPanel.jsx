...
export default function InventoryPanel() {
  ...
  if (!product)
    return (
      <Card>
        <Empty>No products to stock.</Empty>
      </Card>
    );
        ...
        <table className="w-full min-w-[640px] text-sm">
          <thead>
            <tr className="border-b border-line text-left text-[0.6rem] uppercase tracking-[0.16em] text-ink-soft">
              ...
              {product.sizes.map((s) => (
                <th key={s} className="px-3 py-3.5 text-center font-normal">
                  {s}
                </th>
              ))}
            </tr>
                ...
                <td className="px-5 py-3">
                  <div className="flex items-center gap-2.5">
                    <span
                      ...
                      style={{
                        background: `linear-gradient(135deg, ${colour.ramp[2]}, ${colour.ramp[1]} 55%, ${colour.ramp[0]})`,
                      }}
                    />
                    ...
                    <td key={cell.sku} className="px-2 py-2">
                      <input
                        ...
                        onChange={(e) =>
                          setDraft((d) => ({
                            ...d,
                            [cell.sku]: e.target.value.replace(/[^\d]/g, ""),
                          }))
                        }
