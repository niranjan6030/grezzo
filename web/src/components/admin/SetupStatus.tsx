...
import type { IntegrationStatus } from "@/app/api/admin/setup/route";
import { Card } from "./ui";
...
export default function SetupStatus() {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/setup")
      .then((r) => r.json())
      .then((j) => { if (!cancelled) setRows(j.integrations ?? []); })
    ...
    <Card className="mb-8 p-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={load} disabled={busy}
          ...
          {busy ? <Loader2 size={13} className="animate-spin" /> : <RefreshCw size={13} strokeWidth={1.6} />}
          Re-check
        </button>
      </div>
          ...
          {rows.map((r) => {
            ...
            return (
              <div key={r.id} className="flex items-start gap-4 py-3.5">
                <span className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full ${look.cls}`}>
