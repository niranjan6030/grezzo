  ...
  return (
    <>
      ...
      <div className="mb-8 flex gap-6 border-b border-line">
        {([["coupons", "Coupon codes", Ticket], ["bank", "Bank & card offers", CreditCard]] as const)
          .map(([key, label, Icon]) => (
...
const BLANK_COUPON = {
  code: "", description: "", kind: "percent" as Coupon["kind"], value: 10,
  minOrder: "", maxDiscount: "", scopeType: "all" as OfferScope["type"], scopeValue: "",
  startsAt: "", endsAt: "", usageLimit: "", perUserLimit: "1",
  firstOrderOnly: false, active: true,
};

function Coupons() {
  const { data, catalogue, createCoupon, updateCoupon, deleteCoupon } = useAdmin();
  const [form, setForm] = useState<typeof BLANK_COUPON & { id?: string }>(BLANK_COUPON);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    setError(null);
    const rupeesToPaise = (v: string) => (v.trim() === "" ? null : Math.round(Number(v) * 100));
    try {
      const payload = {
        ...(form.id ? { id: form.id } : {}),
        code: form.code,
        ...
        scope: form.scopeType === "all"
          ? { type: "all" } : { type: form.scopeType, value: form.scopeValue },
  ...
  const edit = (c: Coupon) => {
    setForm({
      ...
      firstOrderOnly: c.firstOrderOnly, active: c.active,
    });
    setOpen(true);
    setError(null);
  };
  ...
  return (
    <>
      <div className="mb-6 flex justify-end">
        <Button onClick={() => { setForm(BLANK_COUPON); setOpen(true); setError(null); }}>
          <Plus size={14} strokeWidth={1.8} className="mr-2 inline" /> New coupon
        </Button>
      </div>

      {coupons.length === 0 ? (
        <Card><Empty>No coupon codes yet.</Empty></Card>
      ) : (
            ...
            return (
              <Card key={c.id} className="p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <p className="mt-2 text-sm">
                      {c.kind === "free_shipping" ? "Free delivery"
                        : c.kind === "percent" ? `${c.value}% off` : `${rupees(c.value)} off`}
                    ...
                    {c.description && <p className="mt-1.5 text-xs text-ink-soft">{c.description}</p>}
                  </div>
                ...
                <div className="mt-4 flex flex-wrap gap-x-5 gap-y-1 text-xs text-ink-soft">
                  <span>Used {used}{c.usageLimit !== null ? ` / ${c.usageLimit}` : ""}</span>
                ...
                <div className="mt-5 flex flex-wrap items-center gap-3">
                  ...
                  <Button variant="ghost" onClick={() => updateCoupon({ id: c.id, active: !c.active })}>
                    {c.active ? "Pause" : "Resume"}
                  </Button>
                  <button onClick={() => deleteCoupon(c.id)} aria-label="Delete coupon"
      ...
      <AnimatePresence>
        {open && (
          <Drawer title={form.id ? "Edit coupon" : "New coupon"} onClose={() => setOpen(false)}
            ...
            <Field label="Code" hint="What the shopper types. Letters, numbers and hyphens.">
              <Input required value={form.code} placeholder="FIRST10"
                     onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })} />
            </Field>
            <Field label="Description" hint="Shown in the bag once the code is applied.">
              <Input value={form.description} placeholder="10% off your first order"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {form.kind !== "free_shipping" && (
                <Field label={form.kind === "percent" ? "Percent" : "Amount (₹)"}>
                  <Input required inputMode="numeric" value={String(form.value)}
                         onChange={(e) => setForm({ ...form, value: Number(e.target.value.replace(/[^\d.]/g, "")) })} />
                </Field>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Minimum order (₹)" hint="Blank = no minimum">
                <Input inputMode="numeric" value={form.minOrder}
                       onChange={(e) => setForm({ ...form, minOrder: e.target.value.replace(/[^\d.]/g, "") })} />
              </Field>
              <Field label="Max discount (₹)"
                     hint={form.kind === "percent" ? "Required above 30%" : "Blank = no cap"}>
                <Input inputMode="numeric" value={form.maxDiscount}
            ...
            <Field label="Applies to">
              <Select value={form.scopeType}
                      onChange={(e) => setForm({ ...form, scopeType: e.target.value as OfferScope["type"], scopeValue: "" })}>
            ...
            {form.scopeType !== "all" && (
              <Field label="Which one">
                <Select required value={form.scopeValue}
                  ...
                  {form.scopeType === "collection" && COLLECTIONS.map((c) => <option key={c} value={c}>{c}</option>)}
                  {form.scopeType === "fit" && ALL_FITS.map((f) => <option key={f} value={f}>{f}</option>)}
                  {form.scopeType === "product" && catalogue.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
                </Select>
              </Field>
            )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Total uses" hint="Blank = unlimited">
                <Input inputMode="numeric" value={form.usageLimit}
                       onChange={(e) => setForm({ ...form, usageLimit: e.target.value.replace(/\D/g, "") })} />
              </Field>
              <Field label="Per shopper">
                <Input inputMode="numeric" value={form.perUserLimit}
                       onChange={(e) => setForm({ ...form, perUserLimit: e.target.value.replace(/\D/g, "") })} />
              </Field>
            </div>
...
const RAILS: { id: PaymentMethod; label: string }[] = [
  { id: "card", label: "Card" }, { id: "upi", label: "UPI" },
  { id: "netbanking", label: "Net banking" }, { id: "wallet", label: "Wallet" },
  { id: "emi", label: "EMI" }, { id: "paylater", label: "Pay later" },
];

const BLANK_BANK = {
  bank: "", cardType: "both" as BankOffer["cardType"], network: "any" as BankOffer["network"],
  kind: "percent" as BankOffer["kind"], value: 10, minOrder: "", maxDiscount: "",
  methods: ["card"] as PaymentMethod[], razorpayOfferId: "",
  startsAt: "", endsAt: "", active: true,
};

function BankOffers() {
  const { data, createBankOffer, updateBankOffer, deleteBankOffer } = useAdmin();
  const [form, setForm] = useState<typeof BLANK_BANK & { id?: string }>(BLANK_BANK);
  const [open, setOpen] = useState(false);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...(form.id ? { id: form.id } : {}),
        bank: form.bank, cardType: form.cardType, network: form.network, kind: form.kind,
        value: form.kind === "flat" ? Math.round(Number(form.value) * 100) : Number(form.value),
        minOrderPaise: rupeesToPaise(form.minOrder) ?? 0,
        maxDiscountPaise: rupeesToPaise(form.maxDiscount),
  ...
  const edit = (o: BankOffer) => {
    setForm({
      id: o.id, bank: o.bank, cardType: o.cardType, network: o.network, kind: o.kind,
      ...
      methods: o.methods, razorpayOfferId: o.razorpayOfferId ?? "",
    ...
    <>
      <Card className="mb-6 border-thread/40 bg-thread/8 p-5">
        ...
        <p className="mt-2.5 text-sm leading-relaxed text-ink-soft">
          Card details never reach our servers, so we cannot check whether a shopper
          is really paying with an HDFC credit card. Razorpay can. Create the matching
          Offer in the Razorpay dashboard, paste its id below, and the discount is
          applied against the real card at payment. Without an Offer id the entry
          still shows in the bag — clearly labelled as display-only — but nothing
          is deducted.
        </p>
      </Card>

      <div className="mb-6 flex justify-end">
        <Button onClick={() => { setForm(BLANK_BANK); setOpen(true); setError(null); }}>
          <Plus size={14} strokeWidth={1.8} className="mr-2 inline" /> New bank offer
        </Button>
      </div>

      {offers.length === 0 ? (
        <Card><Empty>No bank or card offers yet.</Empty></Card>
      ) : (
        <div className="grid gap-4 lg:grid-cols-2">
          {offers.map((o) => (
            <Card key={o.id} className="p-5">
              ...
              <div className="mt-5 flex flex-wrap items-center gap-3">
                <Button variant="outline" onClick={() => edit(o)}>Edit</Button>
                <Button variant="ghost" onClick={() => updateBankOffer({ id: o.id, active: !o.active })}>
                  {o.active ? "Pause" : "Resume"}
                </Button>
                <button onClick={() => deleteBankOffer(o.id)} aria-label="Delete offer"
                        className="ml-auto p-2 text-ink-soft transition-colors hover:text-red-700">
                  <Trash2 size={15} strokeWidth={1.5} />
                </button>
              </div>
      ...
      <AnimatePresence>
        {open && (
          <Drawer title={form.id ? "Edit bank offer" : "New bank offer"} onClose={() => setOpen(false)}
                  onSubmit={submit} busy={busy} error={error}
                  submitLabel={form.id ? "Save offer" : "Create offer"}>
            <Field label="Bank">
              <Input required value={form.bank} placeholder="HDFC Bank"
                     onChange={(e) => setForm({ ...form, bank: e.target.value })} />
            </Field>

            <div className="grid grid-cols-2 gap-4">
              <Field label="Card type">
                <Select value={form.cardType}
                        onChange={(e) => setForm({ ...form, cardType: e.target.value as BankOffer["cardType"] })}>
              ...
              <Field label="Network">
                <Select value={form.network}
                        onChange={(e) => setForm({ ...form, network: e.target.value as BankOffer["network"] })}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Type">
                <Select value={form.kind}
                        onChange={(e) => setForm({ ...form, kind: e.target.value as BankOffer["kind"] })}>
                  <option value="percent">Percentage off</option>
                  <option value="flat">Fixed amount off</option>
                </Select>
              </Field>
            ...
            <Field label="Payment methods" hint="Which rails this offer can apply to.">
              <div className="flex flex-wrap gap-2">
                {RAILS.map((r) => (
                  <button key={r.id} type="button" onClick={() => toggleRail(r.id)}
            ...
            <Field label="Razorpay Offer id"
                   hint="From Razorpay dashboard → Offers. Blank makes this display-only.">
              <Input value={form.razorpayOfferId} placeholder="offer_XXXXXXXXXXXX"
...
function Drawer({
  title, children, onClose, onSubmit, busy, error, submitLabel,
}: {
  title: string; children: React.ReactNode; onClose: () => void;
  onSubmit: (e: React.FormEvent) => void; busy: boolean; error: string | null;
  submitLabel: string;
}) {
  return (
    <>
