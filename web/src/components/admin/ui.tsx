...
export function Button({
  children, onClick, variant = "solid", type = "button", disabled, className = "",
}: {
  children: ReactNode; onClick?: () => void; type?: "button" | "submit";
  variant?: "solid" | "outline" | "ghost" | "danger"; disabled?: boolean; className?: string;
}) {
  const styles = {
...
export function Pill({ tone = "neutral", children }: { tone?: "neutral" | "good" | "warn" | "bad"; children: ReactNode }) {
  ...
  return (
    <span className={`inline-block px-2.5 py-1 text-[0.58rem] uppercase tracking-[0.16em] ${styles}`}>
      {children}
    </span>
  );
}
