...
import type { Offer, OfferScope } from "@/lib/admin/types";
  ...
  return (
    <>
      <PanelHead
        ...
        action={
          <Button onClick={() => { setForm(BLANK); setOpen(true); setError(null); }}>
      ...
      {offers.length === 0 ? (
        <Card><Empty>No offers yet. Create one and it applies the moment it saves.</Empty></Card>
      ) : (
            ...
            return (
              <Card key={o.id} className="p-5">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <Pill tone={live ? "good" : "neutral"}>{live ? "Live" : o.active ? "Scheduled" : "Paused"}</Pill>
                </div>

                <p className="mt-3 text-xs text-ink-soft">
                  {o.startsAt ? `From ${new Date(o.startsAt).toLocaleString("en-IN")}` : "Started immediately"}
                  {o.endsAt ? ` · until ${new Date(o.endsAt).toLocaleString("en-IN")}` : " · no end date"}
                </p>
                <p className="mt-1.5 text-xs text-ink-soft">
                  Currently reducing {reach.get(o.id) ?? 0} product{(reach.get(o.id) ?? 0) === 1 ? "" : "s"}.
                </p>
            ...
            >
              ...
              <div className="flex-1 space-y-5 overflow-y-auto px-6 py-7">
                <Field label="Name" hint="Shown on the product card, so write it for shoppers.">
                  <Input required value={form.name} placeholder="Mid-season 20%"
...
function describeScope(s: OfferScope): string {
  switch (s.type) {
    case "all": return "everything";
    case "collection": return `the ${s.value} collection`;
    case "fit": return `${s.value} fits`;
    case "product": return "one product";
  }
}
