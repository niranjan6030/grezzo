...
export default function AdminShell({ children }) {
  ...
  useEffect(() => {
    fetch("/api/admin/session")
      .then((r) => r.json())
      .then((j) =>
        setGate(!j.configured ? "unconfigured" : j.signedIn ? "signed-in" : "signed-out"),
      )
  ...
  return (
    <div className="flex min-h-screen bg-denim-paper">
      <aside className="denim-weave sticky top-0 hidden h-screen w-56 shrink-0 flex-col justify-between px-6 py-8 text-white md:flex">
        <div>
          <Link href="/" className="tracked-lg text-lg">
            GREZZO
          </Link>
    ...
    <div className="flex min-h-screen items-center justify-center px-6">
      <Card className="max-w-lg p-9">
        ...
        <p className="mt-5 text-sm leading-relaxed text-ink-soft">
          Set <code className="bg-denim-wash px-1.5 py-0.5">ADMIN_PASSWORD</code> in your
          environment and restart. Everything else works without it — this only gates the management
          console.
        </p>
