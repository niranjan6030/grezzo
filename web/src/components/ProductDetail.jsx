...
const VIEWS = ["Garment", "Technical", "Fabric"];

export default function ProductDetail({ product, initialColour }) {
  ...
  const [view, setView] = useState("Garment");
  const [added, setAdded] = useState(false);
  const [openSection, setOpenSection] = useState("Composition");
  ...
  const facts = DENIM_FACTS.filter(
    (f) =>
  ...
  const sections = [
    {
      title: "Composition",
      body: `${product.fabric}. ${product.weightOz}oz per square yard${product.stretchPct ? `, ${product.stretchPct}% stretch recovery` : ", no stretch"}.`,
    },
    {
      title: "Fit",
      body: `${product.fit} through the leg with a ${product.rise.toLowerCase()} rise. Model is 185cm with a 32in waist and wears a 32.`,
    },
    {
      title: "Care",
      body: "Machine wash cold, inside out, with like colours. Do not tumble dry — heat is what kills elastane and shrinks cotton. Wash less than you think you need to.",
    },
    {
      title: "Shipping & returns",
      body: "Free delivery on orders over ₹3,000. Dispatched within 24 hours from the nearest warehouse holding your size. Returns free within 30 days, unworn with tags.",
    },
  ];
    ...
    <article className="grid md:grid-cols-2">
            ...
            <button
              ...
              className={`tracked border px-3 py-1.5 transition-colors ${
                view === v
                  ...
                  : "border-denim-deep/40 bg-white/70"
              }`}
            >
              {v}
            </button>
      ...
      <div className="px-5 py-14 md:px-14 md:py-20">
        ...
        <div className="mt-10">
          ...
          {size === null && (
            <p className="mt-3 text-xs text-ink-soft">Select a size to continue.</p>
          )}
        </div>
        ...
        <p className="mt-5 flex items-center gap-2 text-xs text-ink-soft">
          <Truck size={15} strokeWidth={1.25} /> Dispatched in 24 hours from the nearest warehouse
          in stock
        </p>
    ...
    <div className="relative h-full w-full overflow-hidden">
      ...
      <div className="absolute inset-x-0 bottom-0 bg-denim-raw/75 p-5 text-white">
        ...
        <p className="mt-2 text-xs leading-relaxed opacity-80">
          Indigo warp over undyed weft. The pale threads you can see running the other way are why
          this fades instead of going grey.
        </p>
      </div>
    </div>
  );
