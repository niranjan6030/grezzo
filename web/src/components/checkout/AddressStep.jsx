...
export default function AddressStep() {
  ...
  const selectedCount = cart.filter(
    (l) => !deselected.includes(`${l.productId}|${l.colour}|${l.size}`),
  ).length;
    ...
    return (
      <Centered>
        ...
        <p className="mt-4 max-w-md text-sm leading-relaxed text-ink-soft">
          Checkout keeps your addresses and lets you track orders, so it needs an account. Add the
          Firebase configuration — README section 1 — to switch it on.
        </p>
      </Centered>
    );
  }
  ...
  if (!user) {
    return (
      <Centered>
        ...
        <p className="mt-4 max-w-md text-sm leading-relaxed text-ink-soft">
          We keep your delivery addresses and order history against your account, so you only type
          them once.
        </p>
        <Link
          href="/account?next=/checkout/address"
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="tracked flex flex-wrap items-center gap-2">
                  {a.label}
                  {a.isDefault && (
                    <span className="bg-denim-wash px-2 py-0.5 text-[0.55rem] text-ink-soft">
                      Default
                    </span>
                  )}
                </p>
            ...
            >
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  <Input
                    v={form.line1}
                    set={(v) => setForm({ ...form, line1: v })}
                    ph="FLAT / HOUSE, STREET"
                    required
                  />
                </div>
                ...
                <div className="sm:col-span-2">
                  ...
                  {lookup.localities.length > 1 && (
                    <p className="mt-1.5 text-xs text-ink-soft">
                      {lookup.localities.length} localities found for this pincode — start typing to
                      pick one.
                    </p>
                  )}
                </div>
              ...
              {lookup.state === "missing" && form.pincode.length === 6 && (
                <p className="mt-3 text-xs text-ink-soft">
                  We could not look that pincode up — fill in the city and state yourself and the
                  order will be fine.
                </p>
              )}
