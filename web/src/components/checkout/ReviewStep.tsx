...
import { PAYMENT_METHODS, lineKey, type Address, type PaymentMethod } from "@/lib/types";
...
interface Quote {
  totals: {
    subtotalPaise: number; couponCode: string | null; couponDiscountPaise: number;
    shippingPaise: number; codFeePaise: number; totalPaise: number;
  };
  couponError: string | null;
  couponNote: string | null;
  bankOffers: {
    id: string; label: string; methods: PaymentMethod[];
    estimatedPaise: number; applicable: boolean;
  }[];
  codUnavailable: string | null;
}
...
export default function ReviewStep() {
  ...
  const [bankOfferId, setBankOfferId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/addresses")
      .then((r) => r.json())
      .then((j) => setAddress(j.addresses?.find((a: Address) => a.id === addressId) ?? null))
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/razorpay/order", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ cart: payload, addressId, method, code, bankOfferId: selectedBankOffer }),
      });
      const data = await res.json();
      ...
      if (data.configured === false) {
        setError("Stock is reserved and the total is confirmed, but Razorpay keys are not connected yet — see README section 3.");
        return;
      }
      ...
      const only = (m: PaymentMethod) => ({
        upi: m === "upi", card: m === "card", netbanking: m === "netbanking",
        wallet: m === "wallet", paylater: m === "paylater", emi: m === "emi",
      });
      ...
      const rzp = new window.Razorpay({
        ...
        handler: async (response: Record<string, string>) => {
          ...
          else setError("We could not verify that payment. Nothing has been charged twice — contact us with your payment id.");
        },
  ...
  if (!addressId) {
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center text-center">
        ...
        <Link href="/checkout/address" className="tracked mt-8 bg-denim-deep px-10 py-3.5 text-white transition-colors hover:bg-denim-mid">
          Choose address
        </Link>
      </div>
    );
      ...
      <div>
        ...
        <section className="mt-10">
          <div className="flex items-center justify-between border-b border-line pb-3">
            ...
            <Link href="/checkout/address" className="tracked text-ink-soft seam-link">Change</Link>
          </div>
          {address ? (
          ...
          <div className="pt-4">
            {totals?.couponCode ? (
              <div className="flex items-start justify-between gap-3 border border-denim-deep px-4 py-3">
                ...
                <button onClick={() => { setCode(""); setCodeInput(""); }} aria-label="Remove code"
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setCode(codeInput.trim())} disabled={!codeInput.trim()}
              ...
              return (
                <label key={m.id}
                       className={`flex items-start gap-3 border px-4 py-3.5 transition-colors ${
                         disabled ? "cursor-not-allowed border-line opacity-45"
                           : method === m.id ? "cursor-pointer border-denim-deep"
                           : "cursor-pointer border-line hover:border-denim-light"}`}>
                  <input type="radio" name="method" checked={method === m.id} disabled={disabled}
                ...
                {quote.bankOffers.map((o) => (
                  <label key={o.id}
                         className={`flex cursor-pointer items-start gap-3 border p-4 transition-colors ${
                           selectedBankOffer === o.id ? "border-denim-deep" : "border-line hover:border-denim-light"}`}>
                    <input type="radio" name="bankoffer" checked={selectedBankOffer === o.id}
        ...
        <div className="denim-weave-light p-7">
          ...
          <div className="mt-4">
            ...
            {(totals?.couponDiscountPaise ?? 0) > 0 && (
              <Row k={`Coupon ${totals?.couponCode}`}
                   v={`− ${inr(totals!.couponDiscountPaise)}`} good />
            )}
          ...
          <button onClick={placeOrder} disabled={busy || quoting}
          ...
          <Link href="/checkout/address" className="tracked mt-4 block text-center text-ink-soft seam-link">
            Back to delivery
          </Link>
        </div>
      </aside>
