        ...
        >
          <div className="mx-auto max-w-6xl px-5 py-6 md:px-10">
              ...
              {detail && (
                ...
                >
                  <div className="mt-5 grid gap-3 md:grid-cols-2">
                    <Row label="Strictly necessary" desc="Bag, session, security. Cannot be switched off."
                         checked disabled />
                    <Row label="Analytics" desc="Aggregate page and product performance."
                         checked={draft.analytics}
                         onChange={(v) => setDraft((d) => ({ ...d, analytics: v }))} />
                    <Row label="Personalisation" desc="Powers the recommendation model on your browsing."
                         checked={draft.personalisation}
                         onChange={(v) => setDraft((d) => ({ ...d, personalisation: v }))} />
                    <Row label="Marketing" desc="Off by default. Ad measurement across other sites."
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              <button
                onClick={() => setConsent({ analytics: true, personalisation: true, marketing: true })}
                className="tracked bg-denim-deep px-8 py-3 text-white transition-colors hover:bg-denim-mid"
              >
                Accept all
              </button>
              <button
                onClick={() => setConsent({ analytics: false, personalisation: false, marketing: false })}
              ...
              {detail ? (
                ...
                <button onClick={() => setDetail(true)} className="tracked px-4 py-3 seam-link text-ink-soft">
                  Manage
                </button>
              )}
            </div>
...
function Row({ label, desc, checked, disabled, onChange }: {
  ...
  return (
    <label className={`flex items-start gap-3 border border-line p-4 ${disabled ? "opacity-60" : "cursor-pointer"}`}>
