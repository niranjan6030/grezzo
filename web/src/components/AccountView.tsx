    ...
    <section className="mx-auto max-w-xl px-5 py-20 md:px-10">
      ...
      {configured && user && (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }} className="mt-10">
...
function SignIn({ redirectTo }: { redirectTo: string | null }) {
  const {
    signInWithGoogle, signInWithApple, signInWithEmail, createAccount,
    startPhoneSignIn, ensureSession,
  } = useAuth();
  ...
  const submitEmail = () => {
    ...
    if (emailProblem(email)) { setError(null); return; }
    if (mode === "signup") {
      const badPassword = passwordProblem(password);
      if (badPassword) { setError(badPassword); return; }
    }
    run("email", () => mode === "signin"
  ...
  return (
    <div className="mt-10">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <button onClick={() => run("google", signInWithGoogle)} disabled={busy !== null}
      ...
      <div className="mb-6 flex gap-6">
        {([["email", "Email", Mail], ["phone", "Phone", Phone]] as const).map(([key, label, Icon]) => (
          ...
          <div>
            <input
              ...
              className={`field w-full border px-4 py-3.5 outline-none ${
                emailError ? "border-red-400 focus:border-red-500" : "border-line focus:border-denim-deep"}`}
            />
          ...
          {!confirm ? (
            <>
              ...
              <button
                ...
                onClick={() => run("phone", async () => {
                  const confirmFn = await startPhoneSignIn(phone.replace(/\s/g, ""), "grezzo-recaptcha");
              ...
              <p className="text-xs leading-relaxed text-ink-soft">
                Include the country code. We send a six-digit code by SMS — standard
                message rates apply.
              </p>
            </>
          ) : (
            <>
              <input value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, "").slice(0, 6))}
                     inputMode="numeric" autoComplete="one-time-code" placeholder="SIX-DIGIT CODE"
              ...
              <button onClick={() => run("verify", () => confirm(code))}
                      disabled={busy !== null || code.length < 6}
                      className="tracked flex w-full items-center justify-center gap-2 bg-denim-deep py-4 text-white transition-colors hover:bg-denim-mid disabled:opacity-50">
                {busy === "verify" && <Loader2 size={15} className="animate-spin" />}
                Verify and sign in
              </button>
              <button onClick={() => { setConfirm(null); setCode(""); setError(null); }}
                      className="tracked block w-full pt-2 text-ink-soft seam-link">
      ...
      <p className="mt-8 text-xs leading-relaxed text-ink-soft">
        We only ever store what you give us here. Payments are handled by
        Razorpay — card details never touch our servers.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="denim-weave-light mt-10 p-6">
      ...
      <p className="mt-3 text-sm leading-relaxed text-ink-soft">
        ...
        values to <code className="bg-white px-1.5 py-0.5 text-xs">.env.local</code> —
        README section 1 walks through it. Your bag, favourites and
        recommendations all work without an account in the meantime.
      </p>
    </div>
  );
}
    ...
    <>
      ...
      <div className="mt-8">
        ...
        <div className="mt-5 space-y-2 text-sm text-ink-soft">
          <p>{favCount} favourite{favCount === 1 ? "" : "s"} saved.</p>
          <p>
        ...
        <button onClick={() => setConsent({ personalisation: !consent.personalisation })}
...
function GoogleMark() {
  return (
    <svg width="15" height="15" viewBox="0 0 48 48" aria-hidden>
      <path fill="#4285F4" d="M45 24c0-1.6-.1-2.7-.4-3.9H24v7.5h12c-.2 2-1.5 5-4.4 7l6.7 5.2C42.2 36.2 45 30.6 45 24z" />
      <path fill="#34A853" d="M24 46c5.9 0 10.9-2 14.5-5.3l-6.9-5.3c-1.8 1.3-4.3 2.2-7.6 2.2-5.8 0-10.7-3.8-12.5-9.1l-7.1 5.5C8 40.9 15.4 46 24 46z" />
      <path fill="#FBBC05" d="M11.5 28.5c-.5-1.4-.7-2.9-.7-4.5s.3-3.1.7-4.5l-7.1-5.5C2.9 17 2 20.4 2 24s.9 7 2.4 10z" />
      <path fill="#EA4335" d="M24 10.7c3.2 0 5.4 1.4 6.7 2.6l5.9-5.8C33 4.1 29.9 2 24 2 15.4 2 8 7.1 4.4 14l7.1 5.5c1.8-5.3 6.7-8.8 12.5-8.8z" />
    </svg>
  );
}
