...
export default function Recommendations({
  title = "Chosen for you", exclude = [], limit = 4,
}: { title?: string; exclude?: string[]; limit?: number }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setRecs((json.recommendations as Recommendation[]).filter((r) => !exclude.includes(r.productId)).slice(0, limit));
      } catch {
