          ...
          <AnimatePresence initial={false}>
            {priced.map(({ line, key, product, colour }) => {
              ...
              return (
                ...
                >
                  <input type="checkbox" checked={on} onChange={() => toggleSelected(key)}
                         aria-label={`Buy ${product.name} now`}
                         className="mt-1 h-4 w-4 shrink-0 accent-[var(--denim-deep)]" />

                  <Link href={`/product/${product.slug}?c=${colour.code}`}
                  ...
                  <div className={`flex flex-1 flex-col justify-between transition-opacity ${
                    on ? "" : "opacity-45"}`}>
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <Link href={`/product/${product.slug}?c=${colour.code}`}
                              className="tracked seam-link">{product.name}</Link>
                        <p className="mt-1.5 text-xs text-ink-soft">
                          {product.fit} · {colour.wash} · Size {line.size}
                        </p>
                      </div>
                      <button onClick={() => removeLine(line.productId, line.colour, line.size)}
                              aria-label="Remove">
                    ...
                    <div className="mt-4 flex items-center justify-between">
                      <div className="flex items-center border border-line">
                        <button onClick={() => setQty(line.productId, line.colour, line.size, line.qty - 1)}
                                className="px-3 py-2" aria-label="Decrease">
                          <Minus size={13} strokeWidth={1.5} />
                        </button>
                        <span className="min-w-8 text-center text-sm">{line.qty}</span>
                        <button onClick={() => setQty(line.productId, line.colour, line.size, line.qty + 1)}
                                className="px-3 py-2" aria-label="Increase">
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <div className="denim-weave-light p-7">
            ...
            {toFree > 0 && subtotal > 0 && (
              <div className="mt-4">
                ...
                <div className="mt-2 h-[3px] w-full bg-white">
                  <motion.div className="h-full bg-denim-deep"
                    initial={{ width: 0 }}
            ...
            <p className="mt-5 text-[0.62rem] leading-relaxed text-ink-soft">
              Delivery, coupons and payment come next. Nothing is charged until you
