 ...
 * useMemo.
 */
export function useZipProgress(durationMs, enabled, onDone) {
...
export default function ZipCurtain({ progress, openFrom = 0.14, openTo = 0.76, className = "" }) {
  ...
  return (
    <div
      className={`absolute inset-0 overflow-hidden bg-[#0d1420] ${className}`}
...
function Tooth({ top, left, flip, open }) {
  return (
    <span
...
function Slider({ glint }) {
  return (
    <svg width="40" height="66" viewBox="0 0 40 66" fill="none">
      ...
      <path
        d="M8 6h24a4 4 0 0 1 4 4v18a10 10 0 0 1-4 8l-6 4v6H14v-6l-6-4a10 10 0 0 1-4-8V10a4 4 0 0 1 4-4Z"
        fill={`url(#gz-metal-${Math.round(glint * 100)})`}
        stroke="#4a5566"
        strokeWidth="1"
      />
      <rect
