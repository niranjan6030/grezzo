...
const COLUMNS = [
  {
    ...
    links: [
      ["All jeans", "/jeans"],
      ["Core", "/jeans?collection=Core"],
      ["Atelier", "/jeans?collection=Atelier"],
      ["Utility", "/jeans?collection=Utility"],
  ...
  },
  {
    title: "Company",
    links: [
      ["The Denim Index", "/facts"],
      ["Grezzo Lens", "/jeans"],
      ["Account", "/account"],
      ["Privacy", "/contact#privacy"],
      ["Cookies", "/contact#cookies"],
    ],
  },
];
        ...
        <div className="mx-auto grid max-w-6xl gap-12 md:grid-cols-[1.4fr_repeat(3,1fr)]">
          <div>
            ...
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-ink-soft">
              Grezzo is Italian for raw. Menswear denim only — one garment, made properly: woven,
              dyed and finished with the whole process on the label.
            </p>
          </div>
          {COLUMNS.map((col) => (
            <div key={col.title}>
              ...
              <ul className="space-y-2.5">
                {col.links.map(([label, href]) => (
                  <li key={label}>
                    <Link href={href} className="text-sm text-ink-soft seam-link">
                      {label}
                    </Link>
                  </li>
                ))}
