...
import type { DenimFact } from "@/lib/facts";

/** Continuous ribbon of denim facts. Pauses on hover so a fact can be read. */
export function FactRibbon({ facts }: { facts: DenimFact[] }) {
      ...
      <div className="marquee-track">
        {doubled.map((t, i) => (
          <span key={i} className="tracked flex shrink-0 items-center px-7 opacity-90">
            <span className="mr-7 text-thread">✳</span>{t}
          </span>
        ))}
      </div>
    </div>
...
export function FactPanel({
  facts, interval = 7000,
}: { facts: DenimFact[]; interval?: number }) {
  ...
  return (
    <div className="relative min-h-[190px]">
      ...
      <div className="mt-7 flex gap-1.5">
        {facts.map((_, n) => (
          <button key={n} onClick={() => setI(n)} aria-label={`Fact ${n + 1}`}
