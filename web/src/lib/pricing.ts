...
export interface PricedLine {
  productId: string; name: string; colour: string; size: number; qty: number;
  /** product-colour-size, the unit stock is actually held against. */
  sku: string;
  unitPaise: number; linePaise: number;
}
...
export interface PriceOptions {
  /** Already validated by `evaluateCoupon` — pricing does not re-check rules. */
  coupon?: { code: string; discountPaise: number; freeShipping: boolean } | null;
...
export function priceCart(
  ...
  const byId = (id: string) => catalogue.find((p) => p.id === id);
  ...
  for (const line of cart) {
    ...
    lines.push({
      productId: p.id, name: p.name, colour: colour.code, size: line.size, qty,
      sku: skuFor(p.id, colour.code, line.size),
      unitPaise: p.pricePaise, linePaise: p.pricePaise * qty,
    });
  }

  const subtotalPaise = lines.reduce((n, l) => n + l.linePaise, 0);
  const couponDiscountPaise = Math.min(options.coupon?.discountPaise ?? 0, Math.max(subtotalPaise - 100, 0));
...
export function codUnavailableReason(subtotalPaise: number): string | null {
