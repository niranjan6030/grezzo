import type { AdminData, BankOffer, Coupon, OfferScope } from "./admin/types";
...
export interface CouponContext {
  ...
  userId?: string;
  /** Whether this would be the shopper's first order. */
  firstOrder?: boolean;
}
...
const inWindow = (o: { startsAt: string | null; endsAt: string | null }, now: number) =>
  (!o.startsAt || Date.parse(o.startsAt) <= now) &&
  (!o.endsAt || Date.parse(o.endsAt) >= now);

function scopeMatches(scope: OfferScope, p: Product): boolean {
  switch (scope.type) {
    case "all": return true;
    case "collection": return p.collection === scope.value;
    case "product": return p.id === scope.value;
    case "fit": return p.fit === scope.value;
  }
}

/** The part of the basket a scoped coupon is allowed to discount. */
function eligibleSubtotal(coupon: Coupon, ctx: CouponContext): number {
...
export function findCoupon(data: AdminData, code: string): Coupon | undefined {
...
export function liveBankOffers(
  data: AdminData,
  subtotalPaise: number,
  method?: PaymentMethod,
  now = Date.now(),
): BankOffer[] {
  return data.bankOffers.filter((o) =>
    o.active &&
    inWindow(o, now) &&
...
export function bankOfferValue(offer: BankOffer, subtotalPaise: number): number {
  const raw = offer.kind === "percent"
    ? Math.round((subtotalPaise * offer.value) / 100)
    : offer.value;
  return offer.maxDiscountPaise !== null ? Math.min(raw, offer.maxDiscountPaise) : raw;
}

export function describeBankOffer(o: BankOffer): string {
  const amount = o.kind === "percent"
    ? `${o.value}% off`
    : `₹${Math.round(o.value / 100).toLocaleString("en-IN")} off`;
  const cap = o.maxDiscountPaise !== null && o.kind === "percent"
...
export function applyRedemption(
  ...
  const seen = (use as unknown as { orders?: string[] }).orders ?? [];
  if (seen.includes(orderId)) return;
  (use as unknown as { orders: string[] }).orders = [...seen, orderId];
