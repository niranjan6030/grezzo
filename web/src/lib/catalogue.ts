...
import type { AdminData, MergedProduct, Offer } from "./admin/types";
...
function offerApplies(o: Offer, p: Product): boolean {
  switch (o.scope.type) {
    case "all": return true;
    case "collection": return p.collection === o.scope.value;
    case "product": return p.id === o.scope.value;
    case "fit": return p.fit === o.scope.value;
  }
}

function discounted(paise: number, o: Offer): number {
  const off = o.kind === "percent"
 ...
 */
export function mergeCatalogue(data: AdminData, now = Date.now()): MergedProduct[] {
  ...
  return PRODUCTS.map((base) => {
    ...
    let best: { offer: Offer; price: number } | null = null;
    for (const o of live) {
