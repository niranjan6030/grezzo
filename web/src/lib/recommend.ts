...
import type { BrowseEvent, Product } from "./types";
...
const WEIGHT: Record<BrowseEvent["kind"], number> = {
  view: 1, search: 1.2, favourite: 3, add_to_cart: 4, purchase: 5,
};
...
export function localRecommend(
  ...
  const scored = PRODUCTS.filter((c) => !seen.has(c.id)).map((candidate) => {
    let score = 0;
    let bestSource: Product | null = null;
    ...
    for (const [id, w] of engaged) {
      ...
      const contribution = (attr * 0.5 + tag * 0.38 + sameCollection) * priceAffinity * (w / totalWeight);
      score += contribution;
      if (contribution > bestContribution) { bestContribution = contribution; bestSource = src; }
    }
...
function reasonFor(src: Product, dst: Product): string {
