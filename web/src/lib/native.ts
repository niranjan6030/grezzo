/**
 * Native capabilities, when the site is running inside the iOS shell.
 *
 ...
 *  web `getUserMedia` path produces, so Grezzo Lens does not care which ran. */
...
export async function tapFeedback(style: "light" | "medium" = "light"): Promise<void> {
  ...
  try {
    ...
    // Haptics are a nicety; never let them break a purchase.
  }
}
