...
async function localAvailable(sku) {
...
export async function reserveStock(lines, pincode) {
  ...
  if (db) {
    ...
    return {
      ok: Boolean(data?.ok),
      id,
      unavailable: (data?.unavailable ?? []).map((u) => ({
...
export async function commitReservation(reservationId) {
  ...
  if (db) {
    ...
    if (error) {
      console.error("commit_reservation failed", error);
      return false;
    }
    return true;
  }
...
export async function releaseReservation(reservationId) {
  ...
  if (db) {
    ...
    if (error) {
      console.error("release_reservation failed", error);
      return false;
    }
    return true;
  }
...
export async function availabilityFor(productId, colour, sizes) {
  const db = getAdminSupabase();
  ...
  if (db) {
    ...
    if (!error && data) {
      const out = {};
      for (const row of data) out[row.size] = row.available;
      return out;
    }
  }

  const entries = await Promise.all(
    sizes.map(async (s) => [s, Math.max(0, await localAvailable(skuFor(productId, colour, s)))]),
  );
